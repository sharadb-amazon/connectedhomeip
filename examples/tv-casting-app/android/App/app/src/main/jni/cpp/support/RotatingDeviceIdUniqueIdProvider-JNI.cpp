...
CHIP_ERROR RotatingDeviceIdUniqueIdProviderJNI::Initialize(jobject provider)
{
    ...
    VerifyOrReturnValue(mJNIProviderObject != nullptr, CHIP_ERROR_INCORRECT_STATE, ChipLogError(AppServer, "Failed to NewGlobalRef JNIProvider"));
    ...
    mGetMethod = env->GetMethodID(JNIProviderClass, "get", "()[B");
...
MutableByteSpan * RotatingDeviceIdUniqueIdProviderJNI::Get()
{
    ...
    VerifyOrReturnValue(err != CHIP_NO_ERROR, nullptr, ChipLogError(AppServer, "Error calling GetJavaByteByMethod %" CHIP_ERROR_FORMAT, err.Format()));
