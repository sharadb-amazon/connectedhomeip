...
CHIP_ERROR PersistenceManager::WriteAllVideoPlayers(TargetVideoPlayerInfo videoPlayers[])
{
         ...
         videoPlayerIndex++)
    {
        ...
        if (videoPlayer->IsInitialized())
        {
            ...
            ReturnErrorOnFailure(tlvWriter.Put(TLV::ContextTag(kVideoPlayerNumIPsTag), static_cast<uint64_t>(videoPlayer->GetNumIPs())));
            ...
            if(ipAddress != nullptr && videoPlayer->GetNumIPs() > 0)
            {
                ...
                ReturnErrorOnFailure(tlvWriter.StartContainer(TLV::ContextTag(kIpAddressesContainerTag),
                                                              TLV::kTLVType_Structure, ipAddressesContainerType));
    ...
    while ((err = reader.Next()) == CHIP_NO_ERROR)
    {
        ...
        if (videoPlayersContainerTagNum == kIpAddressesContainerTag)
        {
            ...
            while ((err = reader.Next()) == CHIP_NO_ERROR)
            {
                ...
                {
                    char ipAddressStr[Inet::IPAddress::kMaxStringLength];
                    ReturnErrorOnFailure(reader.GetBytes(reinterpret_cast<uint8_t *>(ipAddressStr), Inet::IPAddress::kMaxStringLength));
