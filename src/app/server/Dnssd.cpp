...
CHIP_ERROR DnssdServer::GenerateRotatingDeviceId(char rotatingDeviceIdHexBuffer[], size_t rotatingDeviceIdHexBufferSize)
{
    ...
    Encoding::BytesToUppercaseHexString(rotatingDeviceIdUniqueIdSpan.data(), rotatingDeviceIdUniqueIdSpan.size(), hexRotatingDeviceIdUniqueId, 2 * CHIP_DEVICE_CONFIG_ROTATING_DEVICE_ID_UNIQUE_ID_LENGTH);
    ChipLogProgress(Discovery, "Generating rotating device ID based on unique ID (hex): %s with byte size: %zu", hexRotatingDeviceIdUniqueId, rotatingDeviceIdUniqueIdSpan.size());
