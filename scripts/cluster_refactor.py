...
def extract_command_args_code(function_content):
    ...
    try:
        ...
        print(f"  [WARN] Command arguments block not found in function:\n{function_content[:100]}...")
        return None
...
def add_no_arg_constructor(class_content, class_name):
    """Ensure every processed class gets a no-arg constructor."""
    print(f"  [ADD] Adding default constructor to {class_name}.")

    # Find where to insert the constructor (right after class declaration `{`)
    insertion_point = class_content.find("{") + 1
    no_arg_constructor = f"""
    public {class_name}() {{}}
    \n"""

    return class_content[:insertion_point] + no_arg_constructor + class_content[insertion_point:]


def process_cluster_class(class_content, cluster_name):
    if cluster_name in block_list:
        print(f"[SKIP] Found {cluster_name}Cluster, skipping (in block list).")
        return class_content

    print(f"[PROCESS] Processing {cluster_name}Cluster class...")

    class_content = add_no_arg_constructor(class_content, f"{cluster_name}Cluster")
    ...
    if not functions:
        print(f"  [INFO] No command argument methods found in {cluster_name}Cluster.")
        return class_content
    ...
    # Insert new methods at the end of the class before the last closing brace '}'
    if new_methods:
...
def process_file(file_path):
    try:
        ...
        # Ensure BaseChipCluster gets a default constructor
        if "class BaseChipCluster" in content:
            print("[ADD] Ensuring BaseChipCluster has a default constructor.")
            content = re.sub(
        ...
        output_path = file_path.rsplit('.', 1)[0] + '_modified.' + file_path.rsplit('.', 1)[1]
        with open(output_path, 'w') as file:
            file.write(modified_content)

        print(f"\n[SUCCESS] Modified file written to: {output_path}")
