...
def update_config_file():
    config_file_path = os.path.join(os.path.dirname(__file__), "../build/chip/java/config.gni")
    try:
        with open(config_file_path, 'r', encoding='utf-8') as file:
            config_content = file.read()

        config_content = re.sub(r'(matter_enable_java_compilation\s*=\s*)false', r'\1true', config_content)
        config_content = re.sub(r'(matter_enable_java_generated_api\s*=\s*)false', r'\1true', config_content)
        config_content = re.sub(r'(matter_enable_tlv_decoder_api\s*=\s*)false', r'\1true', config_content)

        with open(config_file_path, 'w', encoding='utf-8') as file:
            file.write(config_content)

        print("[SUCCESS] Updated config.gni flags to true.")
    except FileNotFoundError:
        print(f"[ERROR] Config file '{config_file_path}' not found.")
    except Exception as e:
        print("[ERROR] An error occurred while updating the config file.")
        print(traceback.format_exc())

def update_build_file():
    build_file_path = os.path.join(os.path.dirname(__file__), "../examples/java-matter-controller/BUILD.gn")
    try:
        with open(build_file_path, 'r', encoding='utf-8') as file:
            build_content = file.read()

        if 'import("${chip_root}/build/chip/java/config.gni")' not in build_content:
            build_content = build_content.replace(
                'import("${chip_root}/build/chip/java/rules.gni")',
                'import("${chip_root}/build/chip/java/config.gni")\nimport("${chip_root}/build/chip/java/rules.gni")'
            )
            print("[SUCCESS] Added config.gni import to BUILD.gn file.")

        with open(build_file_path, 'w', encoding='utf-8') as file:
            file.write(build_content)
    except FileNotFoundError:
        print(f"[ERROR] Build file '{build_file_path}' not found.")
    except Exception as e:
        print("[ERROR] An error occurred while updating the BUILD file.")
        print(traceback.format_exc())

def update_args_file():
    args_file_path = os.path.join(os.path.dirname(__file__), "../examples/java-matter-controller/args.gni")
    try:
        with open(args_file_path, 'r', encoding='utf-8') as file:
        ...
        for flag in flags:
            if flag.split(" = ")[0] not in args_content:
                args_content += f"\n{flag}"

        with open(args_file_path, 'w', encoding='utf-8') as file:
            file.write(args_content)
        print("[SUCCESS] Updated args.gni file with required flags.")
    except FileNotFoundError:
        print(f"[ERROR] Args file '{args_file_path}' not found.")
    except Exception as e:
        print("[ERROR] An error occurred while updating the args file.")
        print(traceback.format_exc())

if __name__ == "__main__":
    print("\n[START] Processing ChipClusters.java and updating configurations\n")
    process_file()
    update_config_file()
    update_build_file()
    update_args_file()
